...
class Message {
    ...
    async save() {
        ...
        await DB.post(DB.getKey('messages', this.chatId, 'frame', frame.index), {
            type: 'add',
            ...this
        });
        frame.count++;
    ...
    static async getMessages(chatId, frameId) {
        ...
        messages = messages.filter(
            (message, pos, self) => self.findIndex(msg => msg.id === message.id) === pos
        );
    ...
    static async _getLastFrame(chatId) {
        ...
        if (!frame) {
            ...
            const count = (await DB.getAll(DB.getKey('messages', chatId, 'frame', index), {}, true))
                .length;
