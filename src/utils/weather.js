...
const loadWeather = async (event) => {
    ...
    try {
        const weather = await RPC.request('fetchWeather', { city });
        resultBox.textContent = weather !== null ?
            `Сегодня в ${city} ${weather.temperature}℃` : 'Не удалось загрузить данные о погоде';
    } finally {
